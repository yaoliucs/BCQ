...
class ExtendedReplayBuffer(object):
	def __init__(self, state_dim, action_dim, qpos_dim, qvel_dim, device, max_size=int(1e6)):
		self.max_size = max_size
		self.ptr = 0
		self.size = 0
	...
	def add(self, state, action, next_state, reward, done, qpos, qvel):
		self.state[self.ptr] = state
		self.action[self.ptr] = action
		self.next_state[self.ptr] = next_state
		self.reward[self.ptr] = reward
	...
	def sample_more(self, batch_size):
		ind = np.random.randint(0, self.size, size=batch_size)
		...
		return (
			...
			torch.FloatTensor(self.reward[ind]).to(self.device),
			torch.FloatTensor(self.not_done[ind]).to(self.device),
			torch.FloatTensor(self.qpos[ind]).to(self.device),
			torch.FloatTensor(self.qvel[ind]).to(self.device)
		)
	...
	def save(self, save_folder):
		...
		np.save(f"{save_folder}_reward.npy", self.reward[:self.size])
		np.save(f"{save_folder}_not_done.npy", self.not_done[:self.size])
		np.save(f"{save_folder}_qpos.npy", self.qpos[:self.size])
